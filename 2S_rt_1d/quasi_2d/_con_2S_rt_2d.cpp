...
int main()
{
    ...
    ////// 
    const H5std_string file_name_last("sim_rho050_rhoA044.4_Dt0.1000_alphaA00.0100_sigmaA0.1111_kappa1.0000_etaAA-0.0000_etaAB3.0000_etaBA3.0000_etaBB-0.0000_Lx4096_Ly8_N2909798_n50000_20260112_202243.h5"); ///// 
    ...
    unsigned int R1A0[dims_res_last[1]][dims_res_last[2]] = {0}; 
    unsigned int R2A0[dims_res_last[1]][dims_res_last[2]] = {0}; 
    unsigned int TA0[dims_res_last[1]][dims_res_last[2]] = {0}; 
    unsigned int R1B0[dims_res_last[1]][dims_res_last[2]] = {0}; 
    unsigned int R2B0[dims_res_last[1]][dims_res_last[2]] = {0}; 
    unsigned int TB0[dims_res_last[1]][dims_res_last[2]] = {0}; 
    res_R1A_last.read(R1A0, H5::PredType::NATIVE_UINT, memspace_res_last, dataspace_res_last); 
    res_R2A_last.read(R2A0, H5::PredType::NATIVE_UINT, memspace_res_last, dataspace_res_last); 
    res_TA_last.read(TA0, H5::PredType::NATIVE_UINT, memspace_res_last, dataspace_res_last); 
    res_R1B_last.read(R1B0, H5::PredType::NATIVE_UINT, memspace_res_last, dataspace_res_last); 
    res_R2B_last.read(R2B0, H5::PredType::NATIVE_UINT, memspace_res_last, dataspace_res_last); 
    res_TB_last.read(TB0, H5::PredType::NATIVE_UINT, memspace_res_last, dataspace_res_last); 
    file_last.close(); 
    // 
    ...
    const unsigned int Lx0 = par_i.at("Lx"); 
    const unsigned int Lx = static_cast<unsigned int>(par_i.at("Lx") * 2); // 
    const unsigned int Ly = par_i.at("Ly"); 
    const unsigned int N = static_cast<unsigned int>(par_i.at("N") * 2); 
    const unsigned int NA = static_cast<unsigned int>(par_i.at("NA") * 2); 
    const unsigned int NB = static_cast<unsigned int>(par_i.at("NB") * 2); 
    ...
    vstack(&R1A0[0][0], &R1A0[0][0], &R1A[0][0], Lx0, Lx0, Ly); 
    vstack(&R2A0[0][0], &R2A0[0][0], &R2A[0][0], Lx0, Lx0, Ly); 
    vstack(&TA0[0][0], &TA0[0][0], &TA[0][0], Lx0, Lx0, Ly); 
    vstack(&R1B0[0][0], &R1B0[0][0], &R1B[0][0], Lx0, Lx0, Ly); 
    vstack(&R2B0[0][0], &R2B0[0][0], &R2B[0][0], Lx0, Lx0, Ly); 
    vstack(&TB0[0][0], &TB0[0][0], &TB[0][0], Lx0, Lx0, Ly); 
    // 
    // initialize 
    std::size_t ii = 0; 
    ...
    file_name << "sim_con_t" << t_last << "_rho0" << rho0 << "_rhoA0" << rhoA0 << "_Dt" << std::fixed << std::setprecision(4) << Dt << "_alphaA0" << alphaA0 << "_sigmaA" << sigma_A << "_kappa" << kappa << "_etaAA" << etaAA << "_etaAB" << etaAB << "_etaBA" << etaBA << "_etaBB" << etaBB << std::defaultfloat << "_Lx" << Lx << "_Ly" << Ly << "_N" << N << "_n" << n << "_" << timestamp.str() << ".h5"; 
