...
int main()
{
    ...
    std::filesystem::path subdir_last("00"); ///// 
    ////// 
    const H5std_string file_name_last("sim_rho0100_rhoA0100_Dt0.1000_alphaA00.0100_sigmaA0.1111_kappa1.0000_etaAA-0.0000_etaAB3.0000_etaBA-3.0000_etaBB-0.0000_Lx4_Ly4_N3200_n200_20260120_101548.h5"); ///// 
    ...
    H5::DataSet res_R5A_last = file_last.openDataSet("results_R5A"); 
    H5::DataSet res_R6A_last = file_last.openDataSet("results_R6A"); 
    H5::DataSet res_R7A_last = file_last.openDataSet("results_R7A"); 
    H5::DataSet res_R8A_last = file_last.openDataSet("results_R8A"); 
    ...
    H5::DataSet res_R5B_last = file_last.openDataSet("results_R5B"); 
    H5::DataSet res_R6B_last = file_last.openDataSet("results_R6B"); 
    H5::DataSet res_R7B_last = file_last.openDataSet("results_R7B"); 
    H5::DataSet res_R8B_last = file_last.openDataSet("results_R8B"); 
    ...
    std::vector<unsigned int> R5A(Lx0Ly0); 
    std::vector<unsigned int> R6A(Lx0Ly0); 
    std::vector<unsigned int> R7A(Lx0Ly0); 
    std::vector<unsigned int> R8A(Lx0Ly0); 
    ...
    std::vector<unsigned int> R5B(Lx0Ly0); 
    std::vector<unsigned int> R6B(Lx0Ly0); 
    std::vector<unsigned int> R7B(Lx0Ly0); 
    std::vector<unsigned int> R8B(Lx0Ly0); 
    ...
    res_R5A_last.read(R5A.data(), H5::PredType::NATIVE_UINT, memspace_res_last, dataspace_res_last); 
    res_R6A_last.read(R6A.data(), H5::PredType::NATIVE_UINT, memspace_res_last, dataspace_res_last); 
    res_R7A_last.read(R7A.data(), H5::PredType::NATIVE_UINT, memspace_res_last, dataspace_res_last); 
    res_R8A_last.read(R8A.data(), H5::PredType::NATIVE_UINT, memspace_res_last, dataspace_res_last); 
    ...
    res_R5B_last.read(R5B.data(), H5::PredType::NATIVE_UINT, memspace_res_last, dataspace_res_last); 
    res_R6B_last.read(R6B.data(), H5::PredType::NATIVE_UINT, memspace_res_last, dataspace_res_last); 
    res_R7B_last.read(R7B.data(), H5::PredType::NATIVE_UINT, memspace_res_last, dataspace_res_last); 
    res_R8B_last.read(R8B.data(), H5::PredType::NATIVE_UINT, memspace_res_last, dataspace_res_last); 
    ...
    // const unsigned int save_step = 5; ///// 
    ...
    for (std::size_t jj = 0; jj < Lx; ++jj) {
        for (std::size_t kk = 0; kk < Ly; ++kk) {
            for (std::size_t i = ii; i < (ii + R5A[jj + kk * Lx]); ++i) {
                x[i] = jj; 
                y[i] = kk; 
    ...
    for (std::size_t jj = 0; jj < Lx; ++jj) {
        for (std::size_t kk = 0; kk < Ly; ++kk) {
            for (std::size_t i = ii; i < (ii + R6A[jj + kk * Lx]); ++i) {
                x[i] = jj; 
                y[i] = kk; 
    ...
    for (std::size_t jj = 0; jj < Lx; ++jj) {
        for (std::size_t kk = 0; kk < Ly; ++kk) {
            for (std::size_t i = ii; i < (ii + R7A[jj + kk * Lx]); ++i) {
                x[i] = jj; 
                y[i] = kk; 
    ...
    for (std::size_t jj = 0; jj < Lx; ++jj) {
        for (std::size_t kk = 0; kk < Ly; ++kk) {
            for (std::size_t i = ii; i < (ii + R8A[jj + kk * Lx]); ++i) {
                x[i] = jj; 
                y[i] = kk; 
    ...
    for (std::size_t jj = 0; jj < Lx; ++jj) {
        for (std::size_t kk = 0; kk < Ly; ++kk) {
            for (std::size_t i = ii; i < (ii + R5B[jj + kk * Lx]); ++i) {
                x[i] = jj; 
                y[i] = kk; 
                sigma[i] = ParticleStates::Running_5_state; 
            }
    ...
    for (std::size_t jj = 0; jj < Lx; ++jj) {
        for (std::size_t kk = 0; kk < Ly; ++kk) {
            for (std::size_t i = ii; i < (ii + R6B[jj + kk * Lx]); ++i) {
                x[i] = jj; 
                y[i] = kk; 
                sigma[i] = ParticleStates::Running_6_state; 
            }
    ...
    for (std::size_t jj = 0; jj < Lx; ++jj) {
        for (std::size_t kk = 0; kk < Ly; ++kk) {
            for (std::size_t i = ii; i < (ii + R7B[jj + kk * Lx]); ++i) {
                x[i] = jj; 
                y[i] = kk; 
                sigma[i] = ParticleStates::Running_7_state; 
            }
    ...
    for (std::size_t jj = 0; jj < Lx; ++jj) {
        for (std::size_t kk = 0; kk < Ly; ++kk) {
            for (std::size_t i = ii; i < (ii + R8B[jj + kk * Lx]); ++i) {
                x[i] = jj; 
                y[i] = kk; 
                sigma[i] = ParticleStates::Running_8_state; 
            }
