...
void Snap_GSD_2::dump(int i_step, const std::vector<TPar>& p_arr) {
  if (need_export(i_step)) {
    ...
    gsd_flush(handle_);//Starting from GSD 4.0, gsd_end_frame() does NOT automatically flush buffered frames to the filesystem. Callers must manually call gsd_flush() or gsd_close() to commit the buffered frames to the filesystem.
    delete[] pos;
    delete []type_id;
  }
}
